...
sed 's/ http/ \[arch=amd64,i386\] http/' -i /etc/apt/sources.list
[ -d "/etc/apt/sources.list.d" ] && find /etc/apt/sources.list.d -name "*.list" -type f -exec sed 's/ http/ \[arch=amd64,i386\] http/' -i {} \;
